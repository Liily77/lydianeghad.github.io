...
app.get('/api/produits/:id', (req, res) => {
  Produit.findById(req.params.id).then(p => p ? res.json(p) : res.status(404).json({ message: 'Produit introuvable' }));
});
...
app.get('/auth/connect', (_req, res) => {
  const params = new URLSearchParams({
    ...
    redirect_uri: REDIRECT_URI, scope: 'payments'
  });
...
app.get('/auth/callback', async (req, res) => {
  try {
    ...
    console.log('→ Sandbox access token:', data.access_token);
    // TODO: Persist data.access_token securely
    res.redirect('/admin');
  } catch (err) {
...
app.post('/api/checkout', async (req, res) => {
  try {
    ...
    const response = await axios.post(CHECKOUT_URL, {
      checkout_reference: `order_${Date.now()}`, amount: total, currency: 'EUR',
      shop_name: 'Arc En Ciel', description: 'Commande Arc En Ciel'
...
app.listen(PORT, () => console.log(`🚀 Serveur front+API sur port ${PORT} (Sandbox: ${isSandbox})`));
