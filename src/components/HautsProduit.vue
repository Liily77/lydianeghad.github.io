<template>
  <div class="product-card">
    ...
    <div class="product-info">
      ...
      <p class="product-description" v-if="produit.description">{{ produit.description }}</p>
...
export default {
  ...
  computed: {
    images() {
      ...
      const fallback = this.getImageUrl('/assets/images/image-placeholder.png');

      if (!this.produit.images || this.produit.images.length === 0) {
        return [fallback];
      }

      // Map des URLs via le helper fourni par le parent
      const mapped = this.produit.images
...
/* l'image s'anime grâce aux transitions ci-dessus */
.product-image {
  width: 100%;
  height: 100%;
  object-fit: cover;
...
@media (max-width: 768px) {
  ...
  .product-description { margin-bottom: 0.2rem; }
  .product-price {
    font-size: clamp(0.75rem, 1.5vw, 0.95rem);
    margin: 0;
    padding-left: 0.6rem;
